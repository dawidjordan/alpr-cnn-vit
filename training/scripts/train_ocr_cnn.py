...
def build_model(arch: str, pretrained: bool) -> OCRModel:
    ...
    model = OCRModel(
        backbone=backbone,
        num_classes=NUM_CHARS,      # 108 klas znaków CCPD
        hidden_dim=256,
        dropout=0.3,
    )
...
def main():
    ...
    parser.add_argument("--split-train", type=str, default=None,
                        help="Plik .txt z listą próbek treningowych (walidacja krzyżowa)")
    parser.add_argument("--split-val",   type=str, default=None,
                        help="Plik .txt z listą próbek walidacyjnych (walidacja krzyżowa)")
    parser.add_argument("--ccpd-raw",    type=str, default="data/raw/ccpd",
                        help="Katalog surowych obrazów CCPD (dla --split-train/val)")
    ...
    if args.split_train and args.split_val:
        
        from utils.dataset_ccpd import RawCCPDOCRDataset
        train_dataset = RawCCPDOCRDataset(
            raw_dir=args.ccpd_raw,
            split_file=args.split_train,
            max_samples=args.max_samples,
        )
        val_dataset = RawCCPDOCRDataset(
            raw_dir=args.ccpd_raw,
            split_file=args.split_val,
        )
    else:
